    ...
    public partial class TemplateManagementWindow : Window
    {
        ...
        private void TemplatesList_MouseMove(object sender, System.Windows.Input.MouseEventArgs e)
        {
            if (e.LeftButton == System.Windows.Input.MouseButtonState.Pressed && _draggedItem != null)
            {
                ...
                {
                    _isDraggingNow = true;
                    var template = _draggedItem.DataContext as ViewModels.TemplateItemVM;
                    if (template != null)
                    {
                        TemplatesList.SelectedItem = template;
                        System.Windows.DataObject dragData = new System.Windows.DataObject("TemplateItemVM", template);
                        
                        var dragWindow = CreateDragVisual(_draggedItem, "Plantilla: " + template.Model.Content);
                        dragWindow.Show();
        ...
        {
            if (e.Data.GetDataPresent("TemplateItemVM"))
            {
                var dropped = e.Data.GetData("TemplateItemVM") as ViewModels.TemplateItemVM;
                var item = FindVisualParent<System.Windows.Controls.ListBoxItem>(e.OriginalSource as DependencyObject);
                if (dropped != null && this.DataContext is ViewModels.TemplateManagementViewModel vm)
                {
                    int oldIdx = vm.Templates.IndexOf(dropped);
                    int newIdx = item != null ? vm.Templates.IndexOf((ViewModels.TemplateItemVM)item.DataContext) : vm.Templates.Count - 1;
                    if (newIdx != -1 && oldIdx != newIdx)
                    {
                        vm.Templates.Move(oldIdx, newIdx);
                    }
                }
            }
        }
